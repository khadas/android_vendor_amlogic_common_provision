...
def derive_one_pfpk(file_dgpk2, file_pfid, out_dir, idx, key_tool, soc):
	...
	pfpk = os.popen(cmd).read()

	file_pfpk = out_dir + '/' + soc + '_' + TARGET_NAME + '_' + idx + '.bin'
	f = open(file_pfpk, 'wb')
	f.write(binascii.unhexlify(pfpk[:32]))
	f.close()

	global DERIVED_FILE_NUM
	DERIVED_FILE_NUM = DERIVED_FILE_NUM + 1
	print(os.path.abspath(file_pfpk) + ' derived successfully')
